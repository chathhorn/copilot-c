...
module Copilot.Compile.C.Copilot2SBV
  ( c2sExpr
  , Inputs(..)
  , Ext
  , ExtQue
...
import qualified Copilot.Compile.C.Queue as Q
import qualified Copilot.Compile.C.Witness as W
...
c2sExpr_ e0 env inputs = case e0 of
    ...
    case M.lookup name env of
      Just Local
          ...
          , localType    = t2
            ...
            let Just p = t2 =~= t1
            in  coerce (cong p) e
      _ -> badUsage $ "Unknown variable: " ++ name
  ...
  _ -> badUsage "c2sExpr: unsupported expression"
--------------------------------------------------------------------------------
...
c2sOp1 op = case op of
  ...
  _              -> badUsage "c2sOp1: unsupported operator."
...
c2sOp2 op = case op of
  ...
  _              -> badUsage "c2sOp2: unsupported operator"
